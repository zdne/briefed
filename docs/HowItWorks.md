...
briefed and the paidbyagent.com website are separate repos (`briefed` and [`paidbyagent-site`](https://github.com/zdne/paidbyagent-site)). briefed never runs Eleventy or touches HTML/CSS/templates — the only thing crossing the repo boundary is a Markdown file. If `paidbyagent-site`'s front-matter schema ever changes, `prepare-post.mjs` needs a matching update, and vice versa; that front-matter shape is the entire interface between the two repos.

The `daily-digest.yml` GitHub Actions workflow (cron `0 4 * * *`, plus manual `workflow_dispatch`) runs `sync` → `digest --friendly`, checks out `paidbyagent-site` (via the `SITE_REPO_PAT` fine-grained PAT, scoped to that repo's Contents only) into `site-repo/`, then runs `.github/scripts/prepare-post.mjs` with `SITE_POSTS_DIR=site-repo/site/posts` to turn the latest friendly digest into an Eleventy post (`<date>-briefing.md`) with front matter (`layout: base.njk`, `title`, `date`, `tags: post`, `summary`) written directly into that checkout, then commits and pushes as `github-actions[bot]`. That push triggers `paidbyagent-site`'s own `deploy.yml`, which builds with Eleventy and deploys to GitHub Pages. The commit only ever touches `site/posts/` in the target repo, so templates/CSS/`eleventy.config.js` there can be edited freely at any time without conflicting with the daily bot commit.
