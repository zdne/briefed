...
## Publishing to paidbyagent.com

The `daily-digest.yml` GitHub Actions workflow (cron `0 4 * * *`, plus manual `workflow_dispatch`) runs `sync` → `digest --friendly` → `.github/scripts/prepare-post.mjs`, which turns the latest friendly digest into an Eleventy post (`site/posts/<date>-briefing.md`) with front matter (`layout: base.njk`, `title`, `date`, `tags: post`, `summary`).

briefed and the paidbyagent.com website are separate repos (`briefed` and [`paidbyagent-site`](https://github.com/zdne/paidbyagent-site)). briefed never runs Eleventy or touches HTML/CSS/templates — the only thing crossing the repo boundary is that Markdown file. If `paidbyagent-site`'s front-matter schema ever changes, `prepare-post.mjs` needs a matching update, and vice versa; that front-matter shape is the entire interface between the two repos.

The `daily-digest.yml` job uploads the generated post as a workflow artifact, then a separate `publish-to-site-repo` job checks out `paidbyagent-site` (via the `SITE_REPO_PAT` fine-grained PAT, scoped to that repo's Contents only), copies the post into `site/posts/`, and commits/pushes as `github-actions[bot]` — the same pattern the workflow already used for committing into its own `site/posts` before the split. That push triggers `paidbyagent-site`'s own `deploy.yml`, which builds with Eleventy and deploys to GitHub Pages. `publish-to-site-repo` only ever touches `site/posts/` in the target repo, so templates/CSS/`eleventy.config.js` there can be edited freely at any time without conflicting with the daily bot commit.

`data/agentic-payments-graph.yaml` (curated via the `graph-candidates`/`graph-audit-sources` CLI commands below) is authored here but also needs to exist in `paidbyagent-site` for `map.njk` to render. That sync is manual, not automated: after running either graph command and reviewing the diff, copy the updated file into a `paidbyagent-site` checkout and push it there — that push triggers a normal rebuild via its `deploy.yml`.

*(Migration note, remove once complete: as of the split's Phase 1, `briefed` still also builds and deploys its own copy of `site/` to GitHub Pages under `paidbyagent.com` directly — the cross-repo publish above runs in addition to that, feeding `paidbyagent-site`'s default `*.github.io` URL for testing. Once the site is verified there and the domain is cut over, `briefed`'s own `site/`, `eleventy.config.js`, and the local build/deploy job will be removed, per the split plan.)*
