...
```json
{
  ...
  "briefing": {
    ...
    "optionalTopics": ["MCP", "AI observability"]
  }
}
```

`requiredTopics` always appear in the briefing with explicit "no signal" when nothing matches. `optionalTopics` appear only when there is meaningful signal.
